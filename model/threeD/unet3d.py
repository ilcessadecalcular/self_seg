...
class Only3dUnet(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, init_features=16):
        super(Only3dUnet, self).__init__()
        # self.down=nn.Conv2d(in_feat,mid_feat,3,stride=4,padding=1)
        self.unet_seg = UNet3D(in_channels, out_channels, init_features)
    def forward(self, x):
        #x:b,d,c,h,w
        input = x.permute(0,2,1,3,4)
        #input:b,c,d,h,w
        out = self.unet_seg(input)
        real_out = out.permute(0,2,1,3,4)
...
def get_seg_model(in_channels, out_channels, init_features, **kwargs):
    model = Only3dUnet(in_channels, out_channels, init_features, **kwargs)
    model.init_weights()
