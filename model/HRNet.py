...
from model.twoD.bn_helper import  relu_inplace
import numpy as np
import torch.nn.functional as F
from model.twoD.config import HRNet48, HRNet32
...
class OnlyHRNetSeg(nn.Module):
    def __init__(self, config, in_feat=18, out_feat=1):
        ...
        #     nn.BatchNorm2d(in_feat, momentum=BN_MOMENTUM),
        ...
        self.bn1 = nn.BatchNorm2d(in_feat, momentum=BN_MOMENTUM)
        self.conv2=nn.Conv2d(in_feat, in_feat, 3, 1, 1)
        self.bn2 = nn.BatchNorm2d(in_feat, momentum=BN_MOMENTUM)
...
def main():
    x=torch.ones([1,20,1,256,256],dtype=torch.float32).cuda()
    #only_twod=OnlyHRNetSeg(HRNet48).to('cuda:0')
    #output=only_twod(x)
    model=get_seg_model(HRNet48).to('cuda:0')
    output=model(x)
    print(output.shape)
